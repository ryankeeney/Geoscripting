...
#' @return A spatiallinesdata frame. Each line connects the
...
ConnectPointsToLines <- function(centroids, lines){
  ...
  begin_coord <- data.frame(centroids$centroids_temp2.x,
                            centroids$centroids_temp2.y)
  ...
  #Create spatial lines from the two coordinates dataframes. There is a loop but
  #I don't entirely understand how it works.
  #Credit: https://stackoverflow.com/questions/20531066/convert-begin-and-end-
  #coordinates-into-spatial-lines-in-r
  l <- vector("list", nrow(begin_coord))
  for (i in seq_along(l)) {
    l[[i]] <- Lines(list(Line(rbind(begin_coord[i, ], end_coord[i,]))),
    as.character(i))
  }
